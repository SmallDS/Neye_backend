...
GitHub Actions 在 `main` 推送通过门禁后自动构建并推送镜像。仓库需要配置两个 Actions Secrets：`DOCKERHUB_USERNAME` 和 Docker Hub access token `DOCKERHUB_TOKEN`。镜像名称固定为 `smallds2004/neye-api`，包含以下标签：
...
```bash
docker pull smallds2004/neye-api:latest
```

镜像内置 readiness 健康检查。以下命令默认使用 `smallds2004/neye-api:latest`；正式生产发布可替换为 `smallds2004/neye-api:sha-<完整提交哈希>`，不要用 `latest` 作为唯一可回滚标识。
...
API 容器启动入口只启动 NestJS 服务，不读取数据库维护开关，也不会执行 Prisma、seed 或回填。所有数据库维护都通过镜像内的独立脚本 `neye-db` 执行：

| 子命令 | 用途 | 行为 |
| --- | --- | --- |
| `init` | 首次部署空库 | `prisma db push` 后执行幂等 seed |
| `update` | baseline 前的结构升级 | 先执行 `prisma/manual-updates/*.sql`，再执行安全 `prisma db push` |
| `backfill` | 业务数据回填 | 执行账号租户和客户拼音幂等回填 |
| `status` | 诊断 | 执行 `prisma migrate status` |

这些子命令必须通过一次性维护容器显式调用，禁止把 `neye-db` 作为 API 常驻容器入口。
...
复制 `.env.docker.example` 并填写必填项。`init` 创建当前 schema 并初始化管理员；已有同名管理员时会幂等跳过，若同名账号不是管理员则拒绝自动提权。

```bash
docker run --rm --env-file .env.docker \
  --entrypoint neye-db \
  smallds2004/neye-api:latest init
```

任务成功后直接启动 API。生产环境未设置 `SEED_ADMIN_PASSWORD`，或仍使用示例占位密码时，初始化会拒绝执行。
...
docker run --rm --env-file .env.docker \
  --entrypoint neye-db \
  smallds2004/neye-api:latest update
```

`update` 会按文件名顺序执行 `prisma/manual-updates/*.sql`。当前脚本会先为历史 `import_task_rows` 回填确定性的 `idempotency_key`，再执行 `db push`，解决必填列无法直接加到已有数据的问题。所有 SQL 必须可重复执行并经过评审。

`update` 不传递 `--accept-data-loss`。如果 Prisma 报告需要接受数据丢失或仍有无法执行的变更，应立即停止，由 DBA 核对实际 schema diff、受影响数据和恢复点后制定迁移方案。

#### 4.4.1 P2022 / `import_tasks.phase` 缺列恢复

新镜像中的 Prisma Client 和导入任务恢复逻辑会查询 `import_tasks.phase`。如果 API 镜像先于数据库 schema 发布，新代码会因旧库缺列而返回 Prisma P2022。这是发布顺序不一致，不应通过删除表、重建数据库或接受数据丢失来处理。

恢复步骤：

1. 停止全部 API 副本和导入 worker，避免旧、新 worker 并行处理任务。
2. 使用 `pg_dump` 完成生产库备份，并确认备份文件可读、存储空间充足；有条件时先在隔离恢复库演练。
3. 使用本节上方的 `neye-db update` 一次性任务同步旧库。
...
结构查询和 readiness 均成功后，再检查管理端导入任务列表并用一份小文件验证导入；确认无误后恢复其他 API 副本。

### 4.5 baseline 完成后的切换

当前 `neye-db` 不提供 `migrate` 子命令。完成初始 baseline、空库建库验证和生产副本演练后，再通过独立变更新增受保护的 `migrate` 子命令；在此之前统一使用已评审的 `update` 流程。
...
docker run --rm --env-file .env.docker \
  --entrypoint neye-db \
  smallds2004/neye-api:latest backfill
```
...
| `neye-db` 拒绝命令 | 检查子命令是否为 `init`、`update`、`backfill` 或 `status` |
